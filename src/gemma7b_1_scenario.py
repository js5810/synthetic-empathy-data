...
for index in range(2):
    ...
    try:
        query_1 = """
        Generate 10 different hopeless scenarios in which the person in that situation would benefit from receiving empathy from others. Never include ** in your response and use plain text only. Make sure there is absolutely no overlap with scenarios already generated below:
        {}
        ...
        query_2 = """
        Aggressively compress each item in the list so that the original meaning remains but they are much shorter. Use keywords to keep each item around 2 or 3 words. Never include ** in your response and produce plain text only.
        {}
        """.format('\n'.join(new_scenario))
        chat_completion = client.chat.completions.create(
            model="google/gemma-7b-it",
